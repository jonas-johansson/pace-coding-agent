...
1. **Ctrl+V clipboard paste** — the app reads the system clipboard for image data using native OS tools (`pbpaste`/`osascript` on macOS, `wl-paste` on Wayland, `xclip` on X11). This is the primary UX.
...
|----------|------|---------|
| macOS | `osascript` | `osascript -e 'the clipboard as «class PNGf»'` (writes PNG bytes) |
| Linux (Wayland) | `wl-paste` | `wl-paste --type image/png` |
| Linux (X11) | `xclip` | `xclip -selection clipboard -t image/png -o` |
...
Logic:
1. **macOS**: Run `osascript` with AppleScript to extract `«class PNGf»` data, write to a temp file, read it back. If no image data on clipboard, return `null`.
2. **Linux (Wayland)**: Detect via `$WAYLAND_DISPLAY`. Run `wl-paste --type image/png`. If exit code non-zero or empty stdout, return `null`.
3. **Linux (X11)**: Detect via `$DISPLAY`. Run `xclip -selection clipboard -t image/png -o`. If exit code non-zero or empty stdout, return `null`.
4. **Fallback**: Return `null` (no image paste support, user can still use `@image(...)` syntax).

The image is always returned as PNG regardless of original clipboard format — the OS clipboard tools handle conversion.
...
- **Clipboard paste**: Always PNG (OS tools output PNG).
...
~60 lines. Exports `readClipboardImage()`. Platform detection via environment variables (`$WAYLAND_DISPLAY`, `$DISPLAY`, `process.platform`). Uses `child_process.execFile` with a timeout (2 seconds) to avoid hanging.
...
- If image found and ≤ 5 MB, base64-encodes and adds to `pendingImages`.
...
- Reads files, validates ≤ 5 MB, determines MIME from extension, base64-encodes.
- Returns `{ displayText, contentBlocks }` with images before text.
- Merges `pendingImages` (from clipboard paste) with parsed images.
- Clears `pendingImages` after submit.
...
**Constructor options:** Add `onPasteImage?: () => void | Promise<void>` to the options type.
...
In `toResponsesInput`, for user image blocks:
...
Step 8: tsconfig.json        — Add clipboard.ts to includes
Step 9: npm run lint          — Verify no type errors
```
...
| Animated GIF | Accepted; Anthropic uses first frame only, OpenAI may reject |
